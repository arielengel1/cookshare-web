    ...
    if (category) {
      filterOption = {
        $or: [
          ...
          { ingredients: { $regex: category, $options: 'i' } },
          { text: { $regex: category, $options: 'i' } }
        ]
      };
    }

    const posts = await Post.find(filterOption).sort(sortOption).skip(skip).limit(limit).populate('author', 'name profilePic').lean();
